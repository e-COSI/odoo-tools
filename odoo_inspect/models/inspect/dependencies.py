...
class Dependencies(object):
    ...
    """
    def __init__(self, odoo, modules=None, models=None, models_blacklist=None,
    ...
    def _get_models_data(self, models, models_blacklist):
        ...
        """
        res = {}
        # TODO: Fix transient models issue with Odoo 10
        #transient_model = 'osv_memory'
        ...
        #if v(self.odoo.version) <= v('6.0'):
        #    return res
        models_patterns = \
            [pattern2odoo(model) for model in (models)]
        models_blacklist_patterns = \
            [pattern2odoo(model) for model in (models_blacklist)]
        if models:
            model_obj = self.odoo.env['ir.model']
            ...
            for data in model_obj.read(ids, ['model', 'modules', transient_model]):
                if not self._config['show_model_transient']: #and data[transient_model]:
                    continue
                """
                if not self._config['show_model_normal'] \
                        and not data[transient_model]:
                    continue
                """
                res[data['model']] = {
                    'model': data['model'],
                    ...
                    #'transient': data[transient_model],
                }
        return res
    ...
    def make_dot(self):
        ...
        between modules.
            ...
            >>> graph = odoo.inspect.dependencies(['base'], ['res.partner'])
            >>> graph.make_dot()
